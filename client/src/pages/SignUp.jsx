...
export default function SignUp() {
  ...
  const { signup, loginWithGoogle, googleAvailable } = useAuth();
      ...
      <div className="w-full max-w-sm rounded-3xl border border-line bg-card p-8 shadow-xl shadow-ink/5 sm:p-10">
        <div className="text-center">
          ...
          <p className="mt-2 text-faint">
            Six quick questions after this, and every search gets personal.
          </p>
        </div>
        ...
        <form onSubmit={submit} className={`${googleAvailable ? "mt-4" : "mt-8"} flex flex-col gap-4`}>
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-faint">At least 8 characters. Demo app — don’t reuse a real password.</p>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-faint">
          Already have one?{" "}
          <Link to="/signin" className="font-semibold text-accent transition-colors hover:text-accent-dark">
            Sign in
          </Link>
        </p>
      </div>
